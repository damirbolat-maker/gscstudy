    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {tests.map((test) => (
            <Link
              key={test.id}
              href={`/test/${test.slug}`}
              className="card-premium p-8 bg-white/90 backdrop-blur flex flex-col group"
            >
        ...
        <div className="text-center mt-12">
          <Link
            href="/tests"
            className="btn-primary inline-flex items-center gap-2 px-8 py-3 rounded-lg font-bold"
          >
            {h.title}
